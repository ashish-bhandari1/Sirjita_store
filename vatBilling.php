<?php
   ...
   $billProcced = 0;
   if(isset($_SESSION["sessionvatBillNo"]))
   {        
      $billid = $_SESSION["sessionvatBillNo"]; 
      $sessionResult = $mysqli->query("SELECT * from vatbill where billid = '$billid'");
      $sessionResult2 = $mysqli->query("SELECT * from vatbill where billid = '$billid'");
      $billProcced = 1;
     
      if(isset($_SESSION["newCusid"])){
         $newCusid = $_SESSION["newCusid"];
      }
      $initialCusid = $_SESSION["vatInitialCusid"];
   }
   else
   {
      $billSql = $mysqli->query("SELECT billid, billno from vatbill ORDER BY billno DESC");
   ...
   if(isset($_SESSION["vatInitialCusid"]) && $_SESSION["vatInitialCusid"] != "none"){
      $cusId = $_SESSION["vatInitialCusid"];
   ...
   function fill_product($mysqli){
         $output = '';
         $sql = "SELECT * FROM vat order by name ASC LIMIT 1 ";
         $result  = mysqli_query($mysqli, $sql);
...
<html>
   ...
   <body onload=printdate();>
      ...
      <?php
         ...
         ?>
      <div class="col-md-10" style = "background: rgb(252, 235, 255);">         <?php if (isset($_GET['msg'])){
            echo $_GET['msg'];
            }    
            ?> 
            ...
            <div class="billWrapper grid">
               ...
               <input name = 'bill_no' type = 'number' required = "required" value = '<?php echo $billNo; ?>' style = "display:none;">
                  ...
                  <div class="button d-flex justify-content-end">
                     <button type="submit" id="screeningBtn" name = "vatBillingBtn" onclick= "return confirm('Are you sure wanna Add?');" style = "margin-top:-40px">  Add data</button>
                  </div>
               </form>
            </div>
         </div>
         <!-- //displaying Bill  -->               
         <?php 
         if(isset($_SESSION["sessionvatBillshow"]) && $_SESSION["sessionvatBillshow"] == "visible"){ 
            if(isset($_SESSION["sessionvatBillNo"])){ 
         ...
         <section class = "bill" id = "vat">
                  ...
                  <table class="table table-bordered" style="background-color:rgb(255, 245, 238)" >
                     ...
                     <tbody>
                        ...
                        <tr>
                           ...
                           <button style="border:none; background:none" type="submit" name="deleteV" onclick="return confirm('Are you sure you want to delete this Item?');" title="Click here to delete this data"> 
            ...
            <div class = "l_bill">
               <form method = "POST" action="functions/obj.php">
                  ...
                  <div class="button d-flex justify-content-end">
                  <button type="submit" name = "sessionVatBill" onclick= "return confirm('Are you sure?');"  style = "margin-right:40px">  Done</button>
                  </div>
               </form>
            </div>
         </section>               
      ...
      <script>
         $(document).ready(function(){
            $('#brand').change(function(){
               var vatBrand_id = $(this).val();
         
               $.ajax({
                  url:"load_data.php",
                  method:"POST",
                  data:{vatBrand_id:vatBrand_id},
